		       ...
		       enum v4l2_subdev_format which)
{
	...
	fh = kzalloc(sizeof(*fh) + (sizeof(*fh->probe_fmt) +
		     sizeof(*fh->probe_crop)) * 3, GFP_KERNEL);
	if (fh == NULL)
		return -ENOMEM;
	fh->probe_fmt = (struct v4l2_mbus_framefmt *)(fh + 1);
	fh->probe_crop = (struct v4l2_rect *)(fh->probe_fmt + 3);
	...
	v4l2_subdev_call(vdev->vdev_sensor, pad, set_fmt, fh, 0, &fmt, which);

	if (vdev->vdev_sensor->entity.links[0].sink->entity ==
	   &isp->isp_ccp2.subdev.entity) {
		/* Set format for CCP2 */
		v4l2_subdev_call(&isp->isp_ccp2.subdev, pad, set_fmt, fh,
		...
		/* Set format for CSI2 */
		v4l2_subdev_call(&isp->isp_csi2a.subdev, pad, set_fmt, fh,
	...
	v4l2_subdev_call(&isp->isp_ccdc.subdev, pad, set_fmt, fh,
	...
	if (pix_output->pixelformat != V4L2_PIX_FMT_SGRBG10) {
		...
		v4l2_subdev_call(&isp->isp_prev.subdev, pad, set_fmt, fh,
		...
		v4l2_subdev_call(&isp->isp_res.subdev, pad, set_fmt, fh,
			...
			struct v4l2_fract *best_ival)
{
	...
	for (fmtd_index = 0; ; fmtd_index++) {
		...
		for (size_index = 0; ; size_index++) {
			u32 pixel_format = mbus_code_to_pixfmt(code.code);
			struct v4l2_subdev_frame_size_enum fse;
			struct v4l2_pix_format pix_tmp_in, pix_tmp_out;
			int ival_index;

			fse.index = size_index;
			fse.code = code.code;
			fse.pad = 0;
			...
			pix_tmp_in.pixelformat = pixel_format;
			pix_tmp_in.width = fse.min_width;
			pix_tmp_in.height = fse.min_height;
...
#define TIMEPERFRAME_AVG_FPS(t) \
	(((t).denominator + ((t).numerator >> 1)) / (t).numerator)
				fie.interval.denominator =
					TIMEPERFRAME_AVG_FPS(fie.interval);
...
#define FPS_ABS_DIFF(fps, ival) abs(fps - (ival).denominator / (ival).numerator)
				...
				if (FPS_ABS_DIFF(fps, fie.interval)
...
do_it_now:
				*best_ival = fie.interval;
				best_pix_out = pix_tmp_out;
				best_pix_in->width = fie.width;
				best_pix_in->height = fie.height;
				best_pix_in->pixelformat = pixel_format;
				  ...
				  struct v4l2_frmsizeenum *frms)
{
	...
	fse.index = frms->index;
	fse.code = -1;
	rval = v4l2_subdev_call(vdev->vdev_sensor, pad, enum_frame_size, NULL,
				&fse);
	if (rval < 0)
		goto done;
	...
	pix_out.width = fse.min_width;
	pix_out.height = fse.min_height;
				      ...
				      struct v4l2_frmivalenum *frmi)
{
	...
	for (i = 0, max_dist = (unsigned int)-1; ; ++i) {
		 ...
		 */
		dist = min(fse.min_width, frmi->width)
		     * min(fse.min_height, frmi->height);
		dist = fse.min_width * fse.min_height
		     + frmi->width * frmi->height
		     - 2*dist;

		if (dist < max_dist) {
			width = fse.min_width;
			height = fse.min_height;
			max_dist = dist;
		}
	}
	...
	fie.index = frmi->index;
	fie.pad = 0;
	fie.code = -1;			/* ISP does format conversion */
	fie.width = width;
	fie.height = height;
	rval = v4l2_subdev_call(vdev->vdev_sensor, pad, enum_frame_interval,
				NULL, &fie);

	frmi->type = V4L2_FRMIVAL_TYPE_DISCRETE;
	frmi->discrete = fie.interval;
...
omap34xxcam_video_init(struct isp_video *video)
{
	...
	ret = v4l2_subdev_call(vdev->vdev_sensor, pad, get_fmt, NULL, 0,
			       &format, V4L2_SUBDEV_FORMAT_ACTIVE);
	if (ret) {
		dev_err(&vdev->video.video.dev,
			    ...
			    struct v4l2_subdev_i2c_board_info *board_info)
{
	...
	rval = v4l2_subdev_call(vdev->vdev_sensor, pad, get_fmt, NULL, 0,
				&format, V4L2_SUBDEV_FORMAT_ACTIVE);
